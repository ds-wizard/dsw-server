...
gfnRange ((_, from):(_, to):(_, step):_) = return $ toGVal $ makeRange (T.unpack . asText $ from) (T.unpack . asText $ to) (T.unpack . asText $ step)
...
makeRange from to step = case (readMaybe from :: Maybe Integer, readMaybe to :: Maybe Integer, readMaybe step :: Maybe Integer) of
...
gfnIndexOf ((_, lst):(_, txt):_) = return $ toGVal $ elemIndex (T.unpack . asText $ txt) (map (T.unpack . asText) . fromMaybe [] . asList $ lst)
...
toRoman x
  ...
  | x >= 100 = let (q, r) = x `divMod` 100 in digit 'C' 'D' 'M' q ++ toRoman r
  | x >= 10 = let (q, r) = x `divMod` 10 in digit 'X' 'L' 'C' q ++ toRoman r
  | otherwise = digit 'I' 'V' 'X' x
  where
    digit :: Char -> Char -> Char -> Int -> String
    digit x y z k =
      [[x], [x, x], [x, x, x], [x, y], [y], [y, x], [y, x, x], [y, x, x, x], [x, z]] !! (k - 1)

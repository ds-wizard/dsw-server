...
removeOrphanUuidInChapter km ch =
  km & chaptersM . at (ch ^. uuid) ?~ (ch & questionUuids .~ newQuestionUuids)
  where
    newQuestionUuids = fmap getEntityUuid . catMaybes . fmap (\qUuid -> M.lookup qUuid (km ^. questionsM)) $ (ch ^. questionUuids)

removeOrphanUuidInQuestion :: KnowledgeModel -> Question -> KnowledgeModel
removeOrphanUuidInQuestion km q =
  km & questionsM . at (getEntityUuid q) ?~ (applyToChildren q)
  where
    applyToChildren (OptionsQuestion' q) = OptionsQuestion' $ q & answerUuids .~ newAnswerUuids
      where
        newAnswerUuids = fmap getEntityUuid . catMaybes . fmap (\ansUuid -> M.lookup ansUuid (km ^. answersM)) $ (q ^. answerUuids)
    applyToChildren (ListQuestion' q) = ListQuestion' $ q & itemTemplateQuestionUuids .~ newQuestionUuids
      where
        newQuestionUuids = fmap getEntityUuid . catMaybes . fmap (\qUuid -> M.lookup qUuid (km ^. questionsM)) $ (q ^. itemTemplateQuestionUuids)
    applyToChildren q = q
